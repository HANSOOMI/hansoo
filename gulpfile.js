...
gulp.task("clear", () =>
  del([
    /*dist_folder+"css/", dist_folder+"js/"*/
  ])
);

gulp.task("sass", () => {
  return gulp
    .src([src_scss_folder + "**/!(_)*.scss"], { since: gulp.lastRun("sass") })
...
gulp.task("images", () => {
  return gulp
    .src([src_image_folder + "/**/*.+(png|jpg|jpeg|gif|svg|ico)"], {
...
gulp.task("build", gulp.series("clear", "sass", "js"));
...
gulp.task("watch", () => {
  ...
  const watch = [src_scss_folder + "**/*.scss"];

  gulp.watch(watch, gulp.series("dev")).on("change", browserSync.reload);

  gulp
    .watch(src_common_scss_folder + "**/*.scss")
